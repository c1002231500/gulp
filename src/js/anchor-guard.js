...
define(function(require, exports, module) {
	...
	module.exports = {
		...
		init : function(data) {
			...
			_this.price = eval("params." + rtype + ".guard");
		...
		getGuardList : function(data) {
			...
			if (data != null) {
				...
				$.each(data, function() {
					...
					htm += '<li><div class="guardYou"><div class="gback glv1"><img src="' + _self[0].himage + '" class="img2"/>\
		                        <span class="img3">'
							+ _self[0].days + '<br />\u5929</span>\
		                        <img src="/static_data/images_css/guardbox/' + _self[0].levelImage
							+ '" class="img1"/>\
		                        </div>\
		                        <div class="gname"><a href="#"  title="' + decodeURIComponent(_self[0].name)
							+ '">' + decodeURIComponent(_self[0].name) + '</a></div></div></li>';
				})
			}
			if (n < 8) {
				for (var k = 1; k <= 6 - n; k++) {
		...
		onMessage : function(data) {
			swf.guardLevelup({
				...
				nickname : decodeURIComponent(data.name)
			});
		}
	}
})