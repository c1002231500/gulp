...
$(function() {
    ...
    function cm (pam){
        $.ajax({
            type : "POST",
            url : "/rest/usersStatistics/statisticsAll.mt?action="+pam,
            data : "",
            cache : false,
            dataType : "json",
            async : false
    ...
    $(document).on('mousedown', function(event) {
		...
		if (et.hasClass('toggleBox') || et.parents('.toggleBox')[0]) {
			...
			$('.aud').removeClass('on');
			if(UIF.handler != null && UIF.handler.intervals != null){
				clearInterval(UIF.handler.intervals);
				UIF.handler.intervals = null;
			}
		}
	});
});
